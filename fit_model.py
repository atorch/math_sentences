...
from constants import CHARACTERS, EQUALITIES, NUMBERS, OPENINGS, OPERATIONS
...
def main(epochs=100):
    ...
    # Let's see how the model does when extrapolating to an unseen sentence
    #  (unseen but _similar_ to those in the training set)
    unseen_sentence = "Hey, this is a sentence you've never seen before. What is one + one"
    unseen_result = 2
    print_prediction(model, character_label_encoder, unseen_sentence, unseen_result)

    # Now let's see how the model does when extrapolating to a unseen sentence
    #  that is completely different from the training set (the model has never seen negative numbers!)
    unseen_sentence = "Hey, this is a sentence you've never seen before. What is negative one plus negative two?"
    unseen_result = -3
    print_prediction(model, character_label_encoder, unseen_sentence, unseen_result)

    # See the sentence construction logic in simulate_sentence
    n_possible_sentences = len(OPENINGS) * (len(NUMBERS) ** 2) * len(OPERATIONS) * len(EQUALITIES)

    print(f"Note: the maximum number of possible (unique) sentences generated by the training generator is {n_possible_sentences}")
